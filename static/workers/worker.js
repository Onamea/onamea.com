...
var mnemonicToPrivateKey = (mnemonic) => {
...
var generateKeyPair = (cryptoName, getPublicKey4, keygen3) => (mnemonicLength) => {
  if (mnemonicLength) {
    ...
    const seed = mnemonicToSeedSync(mnemonicDisplay);
    const hdkey = HDKey.fromMasterSeed(seed);
...
var keyPairFromMnemonic = (keyPairFromPrivateKey6) => (mnemonic) => {
...
var keyPairFromMnemonic3 = keyPairFromMnemonic(keyPairFromPrivateKey3);
...
worker.onmessage = async (event) => {
  ...
  while (match === false) {
    ...
    const keyPair = await generateKeyPair6(cryptoName, shouldGenerateMnemonic, xPub, index);
